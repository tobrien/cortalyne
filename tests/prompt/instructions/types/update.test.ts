...
describe('Update Type Instructions', () => {
    beforeEach(async () => {
        jest.clearAllMocks();
        ...
        UpdateType = await import('../../../../src/prompt/instructions/types/update');
    });

    it('exports the correct instruction string', () => {
        // Verify the instruction content contains expected key phrases
        expect(UpdateType.INSTRUCTION).toContain('Task #1');
        expect(UpdateType.INSTRUCTION).toContain('Update');
    });
    ...
    it('creates instructions correctly', async () => {
        ...
        const result = await UpdateType.create(configDir, { customizeContent: mockCustomizeContent });

        // Verify customizeContent was called with the correct arguments
        expect(mockCustomizeContent).toHaveBeenCalledWith(
            configDir,
            constants.DEFAULT_TYPE_INSTRUCTIONS_DIR + '/update.md',
