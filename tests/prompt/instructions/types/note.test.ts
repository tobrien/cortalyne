...
describe('Note Type Instructions', () => {
    beforeEach(async () => {
        jest.clearAllMocks();
        ...
        NoteType = await import('../../../../src/prompt/instructions/types/note');
    });

    it('exports the correct instruction string', () => {
        // Verify the instruction content contains expected key phrases
        expect(NoteType.INSTRUCTION).toContain('Task #1');
        expect(NoteType.INSTRUCTION).toContain('Note');
    });
